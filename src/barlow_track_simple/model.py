...
class BarlowTwinsDualLoss(nn.Module):
    ...
    def forward(self, z1: torch.Tensor, z2: torch.Tensor):
        ...
        z1_0 = z1 - z1.mean(1, keepdim=True) / (z1.std(1, keepdim=True) + self.eps)
        z2_0 = z2 - z2.mean(1, keepdim=True) / (z1.std(1, keepdim=True) + self.eps)

        c_obj = torch.matmul(z1_0, z2_0.T) / z1.shape[1]
